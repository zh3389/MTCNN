...
def convert_to_square(boxlist):
    '''
    :param boxlist: 传入一堆盒子
    :return: 返回这堆盒子以中心点扩充, 最大边长为边长的正方形
    '''
    square_box = boxlist.copy()
    if boxlist.shape[0] == 0:
        return np.array([])
    h = boxlist[:, 3] - boxlist[:, 1]
    w = boxlist[:, 2] - boxlist[:, 0]
    max_side = np.maximum(h, w)
    square_box[:, 0] = boxlist[:, 0] + w * 0.5 - max_side * 0.5
    square_box[:, 1] = boxlist[:, 1] + h * 0.5 - max_side * 0.5
