...
class FaceDataset(Dataset):
    def __init__(self, path):
        ...
        self.dataset.extend(open(os.path.join(path, "positive_label10w.txt")).readlines())  # 将样本按行添加到 self.dataset里
        self.dataset.extend(open(os.path.join(path, "negative_label30w.txt")).readlines())  # 将样本按行添加到 self.dataset里
        self.dataset.extend(open(os.path.join(path, "medium_label10w.txt")).readlines())  # 将样本按行添加到 self.dataset里
...
if __name__ == '__main__':
    '''
    执行此文件生成数据集, 需要 修改的路径有 两个 第45行的路径 和 16 17 18 行的文件名
    '''
    dataset = FaceDataset("D:/celeba/train_data/12/")  # 做好的数据集的路径
    print(dataset[0])
